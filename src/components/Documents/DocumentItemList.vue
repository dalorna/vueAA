   ...
   <div id="topGen">
        <div class="card-header">
            <div class="card-title">
                <div class="type-name">{{DocumentsType.name}}</div>
       ...
       <div class="overflow" v-if="DocumentsItemsList && DocumentsItemsList.length && DocumentsItemsList.length > 0" >
            <div class="cursor-pointer panel-action-bar bb-1 link" v-for="k in DocumentsItemsList" :id="k.id" v-bind:key="k.id" v-on:click="loadDetails(k.id)" v-bind:class="[DocumentsType != undefined && k.id == DocumentsType.id ? 'highlight' : '', {'hidden' : k.id === '00000000-0000-0000-0000-000000000000'}]"  >
                <span>{{k.name}}</span>
            </div>
       </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        deleteClassification(){
            if(this.DocumentsItemsList.length > 0){
        ...
        loadItems(){
          ...
          this.$store.dispatch('getDocumentsItemsList', this.DocumentsType.id);
        },
        loadDetails(id){
            var vm = this;
            vm.$store.dispatch('clearExcludedObjectList');
            vm.$store.dispatch('setDocumentsItem', id);
            vm.$store.dispatch('setDetailsView', 'DocumentsDetail');
        },
        addItem(){
          if(this.DocumentsType) {
            let vm = this;
            vm.$store.dispatch('newDocumentsInstance');
            vm.$store.dispatch('setDetailsView','DocumentsDetail');
          }
        },
