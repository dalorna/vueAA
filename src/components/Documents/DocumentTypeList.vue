<template>
    <div>
        ...
        <div class="panel-action-bar bb-1" v-if="DocumentsTypesList != undefined && DocumentsTypesList.length == 0">
            <span>There are currently no document categories setup.</span>
        </div>
        <div class="overflow">
            <div class="cursor-pointer panel-action-bar bb-1 link" v-for="k in DisplayTypeList" :id="k.id" v-bind:key="k.id" v-on:click="selectType(k.id)" v-bind:class="DocumentsType !== undefined && k.id === DocumentsType.id ? 'highlight' : ''">
                <span>{{k.name}}</span>
            </div>
        </div>
  ...
  export default {
    ...
    watch:{
      HrScopeInView: function () {
        ...
        this.$store.dispatch('setDocumentsType', '');
        this.loadTypes();
      },
      searchInput: debounce(function (val) {
        ...
        vm.DisplayTypeList = vm.DocumentsTypesList.filter(function (classification) {
          if(vm.typeFilter){
        ...
        if(vm.DisplayTypeList.length > 0) {
          ...
          vm.$store.dispatch('setDocumentsType', '');
        }
      }, 100),
    },
    data() {
    ...
    methods: {
      loadTypes(){
        ...
        vm.$store.dispatch('getDocumentsTypesList').then(() => {
          vm.$store.dispatch('showModal', false);
          vm.DisplayTypeList = vm.DocumentsTypesList;
        });
        this.searchInput = '';
       },
      addTypeChange(){
      ...
      selectType(id){
        const vm = this;
        if(vm.DocumentsType.id !== id){
            vm.$store.dispatch('showModal', true);
            vm.$store.dispatch('setDocumentsType', id);
