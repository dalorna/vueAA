   ...
   <div id="topGen">
        <div class="card-header">
            <div class="card-title">
                <div class="type-name">{{AssetsType.name}}</div>
       ...
       <div class="overflow" v-if="AssetsItemsList && AssetsItemsList.length && AssetsItemsList.length > 0" >
            <div class="cursor-pointer panel-action-bar bb-1 link" v-for="k in AssetsItemsList" :id="k.id" v-bind:key="k.id" v-on:click="loadDetails(k.id)" v-bind:class="[AssetsType != undefined && k.id == AssetsType.id ? 'highlight' : '', {'hidden' : k.id === '00000000-0000-0000-0000-000000000000'}]"  >
                <span>{{k.name}}</span>
            </div>
       </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        deleteClassification(){
            if(this.AssetsItemsList.length > 0){
        ...
        loadItems(){
          this.$store.dispatch("setAssetsItem", '');
          this.$store.dispatch("setDetailsView", '');
          this.$store.dispatch('getAssetsItemsList', this.AssetsType.id);
        },
        loadDetails(id){
            var vm = this;
            vm.$store.dispatch('clearExcludedObjectList');
            vm.$store.dispatch("setAssetsItem", id);
            vm.$store.dispatch("setDetailsView", 'AssetsDetail');
        },
        addItem(){
          if(this.AssetsType) {
            let vm = this;
            vm.$store.dispatch("newAssetsInstance");
            vm.$store.dispatch("setDetailsView",'AssetsDetail');
          }
        },
