<template>
    <div>
        ...
        <div class="panel-action-bar bb-1" v-if="AssetsTypesList != undefined && AssetsTypesList.length == 0">
            <span>There are currently no asset categories setup.</span>
        </div>
        <div class="overflow">
            <div class="cursor-pointer panel-action-bar bb-1 link" v-for="k in DisplayTypeList" :id="k.id" v-bind:key="k.id" v-on:click="selectType(k.id)" v-bind:class="AssetsType !== undefined && k.id === AssetsType.id ? 'highlight' : ''">
                <span>{{k.name}}</span>
            </div>
        </div>
  ...
  export default {
    ...
    watch:{
      HrScopeInView: function () {
        ...
        this.$store.dispatch('setAssetsType', '');
        this.loadTypes();
      },
      searchInput: debounce(function (val) {
        ...
        vm.DisplayTypeList = vm.AssetsTypesList.filter(function (classification) {
          if(vm.typeFilter){
        ...
        if(vm.DisplayTypeList.length > 0) {
          ...
          vm.$store.dispatch('setAssetsType', '');
        }
      }, 100),
    },
    data() {
    ...
    methods: {
      loadTypes(){
        ...
        vm.$store.dispatch('getAssetsTypesList').then(() => {
          vm.$store.dispatch('showModal', false);
          vm.DisplayTypeList = vm.AssetsTypesList;
        });
        this.searchInput = '';
       },
      addTypeChange(){
      ...
      selectType(id){
        const vm = this;
        if(id !== this.AssetsType.id){
            this.$store.dispatch('showModal', true);
            vm.$store.dispatch('setAssetsType', id);
